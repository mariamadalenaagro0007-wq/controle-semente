...
async function api(body){
  if(!URL_SCRIPT){
    ...
    if(body.acao === "lancar"){ demoRows.push({data:new Date().toISOString(), usuario:body.usuario, ticket:body.ticket, quantidade:body.quantidade }); return {ok:true}; }
...
async function lancar(){
  ...
  try{
    const r = await api({...sessao, acao:"lancar", ticket, quantidade:parseInt(qtd,10)});
...
async function carregar(){
  try{
    ...
    $("lista").innerHTML = (r.linhas||[]).map(l =>
      `<tr><td>${new Date(l.data).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</td><td>${l.ticket}</td><td>${l.quantidade}</td></tr>`).join("");
  }catch(e){}
}
